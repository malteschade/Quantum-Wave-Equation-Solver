...
def plot_uv(u, v, time, range_u: tuple = (-1, 1), range_v: tuple = (-4000, 4000),
            plot_u=True, plot_v=False) -> go.Figure:
    return _plot_vals(u, v, plot_val1=plot_u, plot_val2=plot_v,
                      val1_range=range_u, val2_range=range_v,
                      title=f'$$Wave \; Field \; t = {np.round(time, 8)}$$',
                      val1_name=r'$u [\mu m]$', val2_name=r'$v [\mu m / s^2]$',
                      x_name=r'$x [m]$')

def plot_state(state, time,  range_u: tuple = (-1, 1), range_v: tuple = (-3000, 3000),
               plot_u=True, plot_v=False) -> go.Figure:
    state_u, state_v = state[:len(state)//2], state[len(state)//2:]
    return _plot_vals(state_u, state_v, plot_val1=plot_u, plot_val2=plot_v,
                      val1_range=range_u, val2_range=range_v,
                      title=f'$$Statevector \; t={np.round(time, 8)}$$',
                      val1_name=r'$TM^{1/2}u$', val2_name=r'$TM^{1/2}v$',
                      x_name=r'$x [m]$')

def plot_medium(mu, rho, range_mu: tuple = (0, 5e10), range_rho: tuple = (0, 5e3),
                plot_mu=True, plot_rho=True) -> go.Figure:
    return _plot_vals(mu, rho, plot_val1=plot_mu, plot_val2=plot_rho,
                      val1_range=range_mu, val2_range=range_rho,
                      title=r'$Medium \; Parameters$',
                      val1_name=r'$\mu [Pa]$', val2_name=r'$\rho [kg/m^3]$',
                      x_name=r'$x [m]$')
...
def plot_heatmap(array: np.ndarray, title='') -> go.Figure:
    ...
    fig.update_layout(
        ...
        xaxis=dict(scaleanchor='y', constrain='domain',
                   title=f'$$Matrix \; Columns ={array.shape[1]}$$'),
...
def plot_multi_v1(idx, times, ode, sim, qpu, rho, mu, bcs):
    ...
    pos_mat = {k: np.zeros(nx+2) for k in ['rho', 'mu']}
    ...
    for key in pos_mat:
        pos_mat[key][1:-1] = locals()[key]
        pos_mat[key][0] = locals()[key][0]
        pos_mat[key][-1] = locals()[key][-1]
    ...
    ax.plot(np.arange(nx+2), rho, color='blue', label='$\\rho$')
    ax.set_ylabel('$\\rho$ [kg/m$^3$]', color='blue')
    ax.tick_params(axis='y', labelcolor='blue')
    ax.set_ylim(1e3, 5e3)
    lines, labels = ax.get_legend_handles_labels()

    ax2.plot(np.arange(nx+2), mu, color='red', label='$\\mu$')
